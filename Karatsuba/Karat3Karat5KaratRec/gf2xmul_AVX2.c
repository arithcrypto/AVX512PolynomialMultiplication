 ...
 * A(x) and B(x) are stored in (3 x TREC_3W/256) 256-bit words
 * @param[out] C Pointer to the result
 * @param[in] A Pointer to the polynomial A(x)
 * @param[in] B Pointer to the polynomial B(x)
 */
int karat_mult3_5(uint64_t *Out, const uint64_t *A, const uint64_t *B) {
	...
	karat_five_way_mult(D3, aa01, bb01);
	karat_five_way_mult(D4, aa02, bb02);
	karat_five_way_mult(D5, aa12, bb12);
