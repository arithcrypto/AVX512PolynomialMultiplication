...
inline static void karat_mult_1(__m128i *C, __m128i *A, __m128i *B) {
	...
	/**************************
		...
		_mm_loadu_si128 			4
		_mm_clmulepi64_si128		9
		_mm_xor_si128				25
		_mm_shuffle_epi32			6
		_mm_setzero_si128			6
		//_mm_storeu_si128			31
		_mm_unpacklo_epi64			3
		_mm_unpackhi_epi64			3
		
		Total						56 without store
...
inline int karatRec(uint64_t * C64, const uint64_t * A64, const uint64_t * B64, int size)
{
	...
	if(size == 1) karat_mult_1((__m128i *)C, (__m128i *)A, (__m128i *)B);
