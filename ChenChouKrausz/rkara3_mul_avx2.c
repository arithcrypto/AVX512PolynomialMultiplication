 ...
 */
 /* Updated by Jean-Marc Robert and Pascal Véron 
 for timings
 */
...
void rkara3_mul_24704_bis(uint64_t *c, const uint64_t *a, const uint64_t *b)
{
//#if 1
  karat_mult3(c,a,b,8192);
  for(int i=0;i<4;i++) c[768+i] = 0;
  madd_128( (uint8_t*)(c+768), (const uint8_t*)(a+384) , (const uint8_t*)(b+384) , 16 );
  madd_128( (uint8_t*)(c+384), (const uint8_t*)(a+384) , (const uint8_t*)b , 3072 );
  madd_128( (uint8_t*)(c+384), (const uint8_t*)(b+384) , (const uint8_t*)a , 3072 );
...
void rkara3_mul_12352_bis(uint64_t *c, const uint64_t *a, const uint64_t *b)
{
//#if 1
	//printf("12352 !\n");
  karat_mult3(c,a,b,4096);
  mul_64( (uint8_t*)(c+384), (const uint8_t*)(a+192) , (const uint8_t*)(b+192) );
  madd_64( (uint8_t*)(c+192), (const uint8_t*)(a+192) , (const uint8_t*)b , 1536 );
  madd_64( (uint8_t*)(c+192), (const uint8_t*)(b+192) , (const uint8_t*)a , 1536 );
/*#else
...
int main()
{
	...
	uint64_t a[400], b[400], c[800], mini, timer=0, START, STOP;
	
	__m256i a256[100],b256[100],c256[200];
	...
	printf("timer rkara3_mul_12288      : %lu\n",timer/NSAMPLES);
	...
	for(int k=0; k<NSAMPLES;k++){
		...
		init_nA_nB((uint64_t*)a256,(uint64_t*)b256);
		
		for(int i=0;i<NTEST;i++)
		{
			
			karat_mult3((uint64_t*)c256,(uint64_t*)a256,(uint64_t*)b256,4096);
	...
	printf("timer rkara3_mul_24576      : %lu\n",timer/NSAMPLES);	
	...
	for(int k=0; k<NSAMPLES;k++){
		...
		for(int i=0;i<NTEST;i++)
		{
			...
			karat_mult3((uint64_t*)c256,(uint64_t*)a256,(uint64_t*)b256,8192);
	...
	printf("timer rkara3_mul_12352      : %lu\n",timer/NSAMPLES);	
	...
	for(int k=0; k<NSAMPLES;k++){
		...
		for(int i=0;i<NTEST;i++)
		{
			...
			rkara3_mul_12352_bis((uint64_t*)c256,(uint64_t*)a256,(uint64_t*)b256);
	...
	printf("timer rkara3_mul_24704      : %lu\n",timer/NSAMPLES);
	...
	for(int k=0; k<NSAMPLES;k++){
		...
		{
			STAMP(START)
			rkara3_mul_24704_bis((uint64_t*)c256,(uint64_t*)a256,(uint64_t*)b256);
			STAMP(STOP)
			if(mini>STOP-START) mini = STOP-START;
