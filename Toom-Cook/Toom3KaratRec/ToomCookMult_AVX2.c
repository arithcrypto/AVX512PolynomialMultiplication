#define T_TM3_3W_256 (T_3W / (4 * WORD))
...
/**
 * @brief Compute C(x) = A(x)*B(x) using TOOM-COOK3 Multiplication 
 *
...
void toom_3_mult(__m256i * Out, const __m256i * A256, const __m256i * B256)
{
	...
	static __m256i tmp[4*(T_TM3_3W_256)];// 
	...
	#if T_TM3_3W_256 == 4
	karat_mult_4( W1, W2, W3);
	#endif
	
	#if T_TM3_3W_256 == 8
	karat_mult_8( W1, W2, W3);
	#endif
	
	#if T_TM3_3W_256 == 16
	karat_mult_16( W1, W2, W3);
	#endif
	...
	//W3 = W3 * W2      ; W2 = W0 * W4
	#if T_TM3_3W_256 == 4
	karat_mult_4( tmp, W3, W2);
	for(int i=0;i<2*(T_TM3_3W_256);i++) W3[i] = tmp[i];
	karat_mult_4( W2, W0, W4);
	

	//W4 = U2 * V2      ; W0 = U0 * V0
	karat_mult_4( W4, U2, V2);
	karat_mult_4( W0, U0, V0);
	#endif
	
	#if T_TM3_3W_256 == 8
	//W3 = W3 * W2      ; W2 = W0 * W4
	karat_mult_8( tmp, W3, W2);
	for(int i=0;i<2*(T_TM3_3W_256);i++) W3[i] = tmp[i];
	karat_mult_8( W2, W0, W4);


	//W4 = U2 * V2      ; W0 = U0 * V0
	karat_mult_8( W4, U2, V2);
	karat_mult_8( W0, U0, V0);
	#endif

	#if T_TM3_3W_256 == 16
	karat_mult_16( tmp, W3, W2);
	for(int i=0;i<2*(T_TM3_3W_256);i++) W3[i] = tmp[i];
	karat_mult_16( W2, W0, W4);
	

	//W4 = U2 * V2      ; W0 = U0 * V0
	karat_mult_16( W4, U2, V2);
	karat_mult_16( W0, U0, V0);
	#endif
	...
	#if T_TM3_3W_256 == 128
	karat_mult_128( tmp, W3, W2);
	for(int i=0;i<2*(T_TM3_3W_256);i++) W3[i] = tmp[i];
	karat_mult_128( W2, W0, W4);
	

	//W4 = U2 * V2      ; W0 = U0 * V0
	karat_mult_128( W4, U2, V2);
	karat_mult_128( W0, U0, V0);
	#endif
	...
	for(int i=0;i<6*T_TM3_3W_256-2;i++){
